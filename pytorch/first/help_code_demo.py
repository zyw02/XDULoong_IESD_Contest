...
class ECG_DataSET():
    def __init__(self, root_dir, indice_dir, mode, size, subject_id=None, transform=None):
        ...
        # if mode == 'test':
        #     self.root_dir = '/root/contest/training_dataset_filter_3f/'
    ...
    def __getitem__(self, idx):
        ...
        ECG_seg = txt_to_numpy(text_path, self.size).reshape(1,1250,1)
        # from sklearn.preprocessing import MinMaxScaler
        # scaler = MinMaxScaler()
        # norm_ecg = scaler.fit_transform(ECG_seg)
        # norm_ecg = norm_ecg.reshape(1,1250)
        label = int(self.names_list[idx].split(' ')[1])
        sample = {'ECG_seg': ECG_seg, 'label': label}
        # sample = {'ECG_seg': ECG_seg, 'label': label, 'name': text_path}
